        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                ...
                if (positionConfig == null) {
                    throw new IllegalArgumentException("Posição não encontrada no mapa: " + i + "," + j);
                }

                var parts = positionConfig.split(",");
                if (parts.length < 2) {
                    throw new IllegalArgumentException("Configuração inválida para posição: " + positionConfig);
                }
